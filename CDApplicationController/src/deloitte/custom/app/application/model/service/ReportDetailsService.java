...
import deloitte.custom.app.application.model.ReportDetails;


/**
 *  Service class that provides CRUD and custom operations against the reportDetails data object.
 *  The behavior of this class is driven by the reportDetails classMappingDescriptor in persistence-mapping.xml.
 *
 ...
 */
public class ReportDetailsService extends EntityCRUDService<ReportDetails> {

    /**
     * Default constructor. If autoQuery is set to true in the classMappingDescriptor in persistence-mapping.xml, then
     * the findAll method will be executed and the reportDetails list will be populated when this constructor is invoked.
     ...
     *
     * ReportDetailsService crudService = (ReportDetailsService) EntityUtils.getEntityCRUDService(ReportDetails.class);
     *
     ...
     */
    public ReportDetailsService(boolean autoQuery) {
        super(autoQuery);
    }
     ...
     */
    public ReportDetailsService(boolean doRemoteReadInBackground, boolean doRemoteWriteInBackground) {
        super(false);
    ...
    /**
     * This method is automatically called when using the Create operation on the reportDetails collection
     * in the data control palette.
     *
     * Note that this method does NOT add the reportDetails to the entity list because this method is
     ...
     * logic there. The CDM EntityList class ensures that this method is called after a reportDetails has been added.
     ...
     * instead to ensure that iterator binding and reportDetails list stay in sync.
     * @param index
     * @param reportDetails
     */
    public void addReportDetails(int index, ReportDetails reportDetails) {
        addEntity(index, reportDetails);
    }

    /**
     * Sets reportDetails state to new, and if addToList argument is true, it adds the reportDetails to the
     * reportDetails list and and fires change event to refresh the list in the UI
     * @param index
     * @param reportDetails
     * @param addToList
     */
    public void addReportDetails(int index, ReportDetails reportDetails, boolean addToList) {
        addEntity(index, reportDetails, addToList);
    }

    /**
     * This method is automatically called when using the Delete operation on the reportDetails collection
     ...
     *
     * Note that this method does NOT remove the reportDetails from the entity list because this method is
     ...
     * logic there. The CDM EntityList class ensures that this refresh method is called after a reportDetails has been added.
     *
     * Do NOT drag and drop this method from the data control palette, use the Delete operation
     * instead to ensure that iterator binding and reportDetails list stay in sync.
     * @param reportDetails
     */
    public void removeReportDetails(ReportDetails reportDetails) {
        removeEntity(reportDetails);
    }


    /**
     * Removes a reportDetails using the configured local and remote persistence managers.
     * If removeFromList argument is true, it removes the reportDetails from the
     * a reportDetails list and and fires change event to refresh the list in the UI
     * @param a reportDetails
     * @param removeFromList
     */
    public void removeReportDetails(ReportDetails reportDetails, boolean removeFromList) {
        removeEntity(reportDetails, removeFromList);
    }

    /**
     * Inserts or updates a reportDetails using the configured persistence managers.
     * The insert or update is determined by calling isNewEntity on the reportDetails instance.
     * @param reportDetails
     */
    public void saveReportDetails(ReportDetails reportDetails) {
        super.mergeEntity(reportDetails);
    }

    /**
     * Retrieves all reportDetails instances using the configured persistence managers and populates the reportDetails list
     * with the result.
     ...
     */
    public void findAllReportDetails() {
        super.findAll();
    }

    /**
     * Retrieves all reportDetails instances using the findAll method on the remote persistence manager
     * and populates the reportDetails list
     */
    public void findAllReportDetailsRemote() {
        super.doRemoteFindAll();
    }

    /**
     * Retrieves the reportDetails instances that match the searchValue filter using the configured persistence
     * managers and populates the reportDetails list with the result.
     ...
     * @param searchValue
     */
    public void findReportDetails(String searchValue) {
        super.find(searchValue);
    }


    /**
     * Resets the values of the reportDetails instance to the values as stored in the SQLite database. This method
     * will do nothing when the reportDetails is not persisted to the database.
     * @param reportDetails
     */
    public void resetReportDetails(ReportDetails reportDetails) {
        super.resetEntity(reportDetails);
    }
